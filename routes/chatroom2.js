...
router.put("/:chatId/", (request, response, next) => {
    ...
    let query = 'SELECT * FROM Members WHERE MemberId = ANY($1::integer[])'
    let values = [request.body.memberIds]
...
console.log(request.decoded)
    ...
    pool.query(query, values)
        ...
        let query = 'SELECT * FROM ChatMembers WHERE ChatId=$1 AND MemberId = ANY($2::integer[])'
...
/**
 * @api {get} /members/:memberId? Request to get the chats of user
 * @apiName GetChats
 * @apiGroup Chats
 * 
 * @apiHeader {String} authorization Valid JSON Web Token JWT
 ...
 * @apiUse JSONError
 */ 
router.get("/members/:memberId/", (request, response, next) => {
    //validate on missing or invalid (type) parameters
